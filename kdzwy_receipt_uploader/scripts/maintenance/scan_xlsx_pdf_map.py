...
def main() -> int:
    ...
    parser.add_argument("--output-dir", type=Path, default=None, help="映射输出目录；公司模式下必须显式指定")
    parser.add_argument("--month-dir", type=Path, default=None, help="单个月份目录；读取该目录下 *.conf")
    ...
    if month_dir is not None:
        ...
        report = match_month_directory(month_dir, config, month_dir / config.output_dirname)
        ...
        print(f"map 输出：{month_dir / config.output_dirname / 'xlsx_pdf_map.json'}")
        return 0

"""Analyze accountId in the legacy request shape for historical evidence only.

The test reads the existing local receipt draft and the previously exported
...
DEFAULT_AUXILIARY_REPORT = PROJECT / "data" / "diagnostics" / "auxiliary_items" / "auxiliary_items_real_api.report.json"
DEFAULT_RECEIPT = PROJECT / "data" / "inbox" / "weiyu" / "7月" / "receipts" / "receipt_26112000002695439356" / "receipt.json"
DEFAULT_SUBJECT_CSV = PROJECT / "data" / "diagnostics" / "live_account_ids.csv"
DEFAULT_OUTPUT = PROJECT / "data" / "diagnostics" / "accountid_auxiliary_relation.report.json"
...
def analyze_request(entries: list[dict[str, Any]], auxiliary_ids: dict[int, set[str]], subject_ids: set[str]) -> dict[str, Any]:
    ...
    return {
        ...
        "conclusion": (
            "entries[].accountId 是科目/账户 ID；辅助核算条目 ID 应通过 supplierId、customId、empId、projectId、deptId、inventoryId 等独立字段传递。"
...
def main() -> None:
    ...
    parser.add_argument("--auxiliary-report", type=Path, default=DEFAULT_AUXILIARY_REPORT)
    parser.add_argument("--receipt", type=Path, default=DEFAULT_RECEIPT)
    parser.add_argument("--subject-csv", type=Path, default=DEFAULT_SUBJECT_CSV)
    ...
    report["sources"] = {
        ...
        "subjectCsv": str(args.subject_csv.resolve()),
    ...
    print("本测试只读本地报告和草稿，未调用保存、上传、更新或删除接口。")

...
def main() -> None:
    with tempfile.TemporaryDirectory() as directory:
        ...
        generate_receipts(
            ...
            sales_map_values={"1001": {"amount": 100.0, "taxAmount": 13.0, "totalAmount": 113.0, "date": "2026-08-31"}},
            template_config={
                "voucher_templates": [{
                    "name": "收客户款",
                    "when": {"itemClass": "客户"},
                    ...
                    "entries": [
                        {"dc": 1, "accountSelector": {"number": "1122"}, "amountFrom": "sales_map.totalAmount", "amountForFrom": "sales_map.totalAmount", "auxiliary": {"field": "customerId", "name": "创界科技有限公司", "itemClassId": 1}},
        ...
        assert voucher["entries"][0]["customerId"] == "c1"
        ...
        assert "auxiliaryExpected" not in voucher["entries"][0]
        assert [entry["accountNumber"] for entry in voucher["entries"]] == ["1122", "5001", "22210106"]
        print("收客户款模板测试通过")


def test_customer_receipt_template() -> None:
    main()

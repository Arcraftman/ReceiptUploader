...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        report = run_company_checks(
            project_root(), args.company, args.month,
        ...
        print('Invalid test configuration: ' + type(exc).__name__, file=sys.stderr)
        return 2
    output = args.output or project_root() / 'runtime/read_api_tests' / args.company / args.month
    write_report(report, output)
    print('Results: ' + ', '.join(f'{k}={v}' for k, v in report['summary'].items()))
    print('Report: ' + str((output / 'RESULTS.md').resolve()))

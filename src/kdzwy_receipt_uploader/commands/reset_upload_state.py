...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Clear local upload checkpoints for a company and month")
    ...
    parser.add_argument("--source", choices=(*SOURCES, "all"), default="all", help="Clear only the selected source; defaults to all sources")
...
def read_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise SystemExit(f"Configuration must be a JSON object: {path}")
    return value
...
def main(argv: list[str] | None = None) -> int:
    ...
    if Path(selector).name != selector:
        raise SystemExit("Company config must be a filename without directories")
    config_name = selector[:-5] if selector.lower().endswith(".json") else selector
    config_path = project_root() / "config" / "companies" / f"{config_name}.json"
    try:
    ...
    try:
        ...
        if len(target_keys) != 1:
            raise ValueError(f"Multiple target accountbooks in the same month: {sorted(target_keys)}")
    ...
    workspace_root = project_root() / workspace_relative_path(login_account, accountbook.key, dataset, month)
    ...
    print(json.dumps({
        ...
        "message": f"{args.source} local upload state cleared; the next confirm uses the current remote voucher number.",
